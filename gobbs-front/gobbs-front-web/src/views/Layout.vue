    ...
    <div class="header" v-if="showHeader">
      <div class="header-content" :style="{ width: proxy.globalInfo.bodyWidth + 'px' }">
          ...
          <template v-for="board in boardList">
            <el-popover placement="bottom-start" :width="300" trigger="hover" v-if="board.children.length > 0">
              ...
              <div class="sub-board-list">
                ...
                ]" v-for="subBoard in board.children" @click="subBoardClickHandler(subBoard)">{{ subBoard.boardName
                  }}</span>
              </div>
            </el-popover>
            <span :class="[
              'menu-item',
              board.boardId == activePboardId ? 'active' : '',
            ]" v-else @click="boardClickHandler(board)">{{ board.boardName }}</span>
          </template>
        </div>
        <!--登录，注册 用户信息-->
        ...
        <div class="user-info-panel">
          <div class="op-btn">
            <el-button type="warning" class="op-btn" @click="goAdmin">
              管理端<span class="iconfont icon-edit"></span>
            </el-button>
          ...
          <template v-if="userInfo.userId">
            <div class="message-info">
              <el-dropdown>
                <el-badge :value="messageCountInfo.total" class="item" :hidden="messageCountInfo.total == null ||
          ...
          <el-button-group :style="{ 'margin-left': '10px' }" v-else>
            ...
            <el-button type="default" plain @click="loginAndRegister(0)">注册</el-button>
          </el-button-group>
        </div>
      </div>
    </div>
...
const goAdmin = () => {
  window.open("http://139.196.10.46:6656", "_blank");
};
